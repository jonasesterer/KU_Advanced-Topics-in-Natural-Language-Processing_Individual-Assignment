...
class MultiHeadAttention(nn.Module):
    def __init__(self, emb_dim: int, num_heads: int):
        super().__init__()
        assert emb_dim >= num_heads, "The embedding dimension cannot be smaller than the number of heads"
        head_dim = emb_dim // num_heads
        
                                #        B           S      num_heads  head_dim -> # B x num_heads x S x head_dim
        self.split = lambda x: x.view(x.size(0), x.size(1), num_heads, head_dim).transpose(1,2) 
                             # -> B x S x num_heads x head_dim # -> B x S x num_heads * head_dim  
                                                 # avoid copy
        self.collect = lambda x: x.transpose(1,2).contiguous().view(x.size(0), x.size(2), num_heads * head_dim)
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.sinusoid_table = nn.Embedding.from_pretrained(get_sinusoid_table(max_len + 3, emb_dim), freeze=True)
        
        self.dropout = nn.Dropout(p=dropout)

        # num_layers transformer blocks
        self.blocks_transformer = nn.ModuleList([TransformerBlock(emb_dim=emb_dim, num_heads=num_heads, dropout=dropout, forward_dim=forward_dim) for _ in range(num_layers)])
    ...
    def forward(self, x, mask=None):
        ...
        position_indices = torch.arange(1, x.size(1) + 1, device=device).unsqueeze(0).expand(x.size(0), -1)
...
class DecoderBlock(nn.Module):
    def __init__(self, emb_dim, num_heads, forward_dim, dropout):
        ...
        self.transformer_block = TransformerBlock(emb_dim, num_heads=num_heads, dropout=dropout, forward_dim=forward_dim) 
        self.dropout = nn.Dropout(p=dropout)
...
class Decoder(nn.Module):
    def __init__(
        ...
        # num_layers transformer blocks
        self.blocks_decoders = nn.ModuleList([DecoderBlock(emb_dim=emb_dim, num_heads=num_heads, dropout=dropout, forward_dim=forward_dim) for _ in range(num_layers)])
