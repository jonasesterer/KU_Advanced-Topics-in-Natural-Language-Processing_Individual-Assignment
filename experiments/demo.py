...
tgt_tokens_all = list(map(lambda x: x.ids, tokenizer.encode_batch(
    tgt_sentences 
)))


# it will auto pad, because I trained it to lol
src_tokens_all = list(map(lambda x: x.ids, tokenizer.encode_batch(
    src_sentences 
)))
...
out = model.forward(src=src_tokens_all, tgt=tgt_tokens_all) # outputs in log space, so to get probabilities, we need to call softmax
